...
/**
 * The single-claimer guarantee relies on {@code SELECT ... FOR UPDATE SKIP LOCKED}
 * (LibraryReservationIntentRepository.findClaimableWaitingForUpdate). H2 does not
 * reproduce that locking faithfully under concurrency — two racing claimers could
 * both read the row, making the H2 version of this test flaky. It therefore runs on
 * a real Postgres (Testcontainers, via {@link AbstractPostgresIT}), where a locked
