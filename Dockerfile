...
# Base: ubuntu:22.04 (digest-pinned, ADR 0062)
FROM --platform=$BUILDPLATFORM ubuntu:22.04@sha256:4f838adc7181d9039ac795a7d0aba05a9bd9ecd480d294483169c5def983b64d AS rusaint-builder
...
# ---- Build stage ----------------------------------------------------------
# Base: eclipse-temurin:21-jdk-jammy (digest-pinned, ADR 0062)
FROM --platform=$BUILDPLATFORM eclipse-temurin:21-jdk-jammy@sha256:801b7e1a9c4befaf82bf9a2a58025ef43a7694bbc84779187ad0524d84742772 AS builder
WORKDIR /workspace
...
# ---- Runtime stage --------------------------------------------------------
# Base: eclipse-temurin:21-jre-jammy (digest-pinned, ADR 0062)
