...
**모든 설계·구현·도구 선택·아키텍처 결정은 아래 세 기준을 이 순서로 검토한다:**

1. **포트폴리오 가치** — 면접에서 설명할 수 있는가? non-obvious한 문제를 해결했는가? 트렌드 기술(MCP, LLM, Zero-Trust, GitOps 등)을 실제 문제에 적용했는가?
2. **현재 트렌드 적합성** — 업계 표준 방식인가? 과도한 추상화나 미래 대비 설계는 아닌가?
3. **현실적 구현 가능성** — 학생 1명이 만들 수 있는 범위인가?

> 기술적으로 옳더라도 포트폴리오 가치가 없으면 채택하지 않는다.
> 의사결정을 기록할 때(`docs/adr/`, `TROUBLESHOOTING.md`) "왜 이 방법이 포트폴리오에 좋은가"를 반드시 남긴다.
...
GitHub contribution과 커밋 페이지에 **오직 사용자(ghdtjdwn)만** 표시되어야 한다.
...
- **author/committer는 반드시 사용자 계정**: `git config user.name=ghdtjdwn`,
