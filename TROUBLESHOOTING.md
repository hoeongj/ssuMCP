...
- 검증: 단위(store 스킵/영속/degrade·코덱 왕복)+리포지토리(복합키 H2 왕복)+풀컨텍스트(V13↔엔티티 validate) green, 전체 스위트 green. **배포·마이그레이션 검증 완료(2026-06-18)**: PR#83 머지→롤아웃, prod에서 `Flyway V13 success(now at version v13)` + `academic_embeddings` 생성 + pod `sha-d44835b` healthy(restarts=0) + 새 store 경로 실행 로그(`PersistentAcademicEmbeddingStore: 216 missing chunk(s)...`) 확인. 단 오늘 쿼터 소진이라 현재 `embeddingActive=false chunks=0`, rows=0. **활성화만 다음 쿼터 리셋(태평양 자정) 후 첫 갱신 대기** → 216청크 영속 시 INFO `academic-policy corpus refreshed ... embeddingActive=true chunks=216` + `SELECT count(*) FROM academic_embeddings`로 확인.
...
> ⚠️ 2026-06-16: 이 기능(LMS STT 전사)은 ADR-0032에 의해 제거됨. 역사적 기록으로 보존.
...
- 맥락:
  ...
  - 사용자 확인 후 bad merge commit `3020809fb09e546cd6f1d14fc69f75755683e2ba`를 `ghdtjdwn <seongjuice999@gmail.com>` author/committer로 amend한 corrected commit `0ea76e66219756942348d975541759526e7321cc`를 만들고 force-push했다.
...
- 핵심 파일/커밋:
  ...
  - PR: `https://github.com/ghdtjdwn/ssuMCP/pull/25`
- 검증:
  - 변경 범위 테스트 통과:
...
- 검증: 다음 2분 사이클에 "Setting new image to ghcr.io/ghdtjdwn/ssumcp:sha-...", "images_updated=1 errors=0" 로그 확인.
...
- 핵심 파일/커밋:
  ...
  - PR: `https://github.com/ghdtjdwn/ssuMCP/pull/57`
- 검증:
...
- 핵심 파일 및 커밋:
  ...
  - PR: `https://github.com/ghdtjdwn/ssuMCP/pull/51`
- 포트폴리오 포인트:
...
## 2026-06-16 - ssuAgent `create_react_agent` 루핑 버그 → manual bind_tools 루프 전환

### 상황

ssuAI 챗봇에서 "도서관 2층 오픈열람실 96번 예약해줘"를 보냈을 때 실제 예약이 이루어지지 않았다. 이전에 기록한 `\r` 오염·fallback 무력화 버그는 모두 fix되어 배포된 상태(`sha-66b9e0d`)였음. 챗봇이 응답은 하지만 예약 승인 UI(HitlCard)가 뜨지 않았다.

### 잘못된 가설

- **처음 가설(세션 시작 시)**: Groq llama-3.3-70b가 `create_react_agent` 내부의 system 프롬프트 때문에 tool call을 Llama XML 형식(`<function=...>`)으로 생성 → Groq API가 `tool_use_failed 400`으로 거부 → 조용히 OpenRouter로 cascade → OpenRouter 약한 모델이 tool call 없이 텍스트만 반환.
  - prod 로그에서 `<function=start_auth(...)>` XML이 1회 관측되어 "원인"으로 추정했음.
- **검증 결과**: pod 내 통제된 A/B 테스트(동일 입력, turn-2 경로: `prepare_*` → `AUTH_REQUIRED` → `start_auth`)에서 **`create_react_agent`도 XML 에러를 내지 않았음**. → XML 인과는 **재현 불가, 미확정**으로 분류.

### 실제 확인된 문제

동일 A/B 테스트에서 **`create_react_agent`가 `prepare_reserve_library_seat`를 2번 호출**했다 (turn-0 성공 후에도 루핑):

```
REACT tool_calls seen: ['prepare_reserve_library_seat', 'start_auth', 'prepare_reserve_library_seat', 'start_auth']
```

이 루핑이 HITL 흐름에 미치는 영향:
1. `prepare_*`가 2번 호출되면 `actionId`가 2개 생성됨.
2. `_extract_action_id`는 **최근 10개 ToolMessage를 역순으로** 스캔하여 첫 번째 `actionId`를 반환.
3. 두 번째 `prepare_*` 결과가 최신이므로 2번째 `actionId`로 승인 게이트가 걸림.
4. 1번째 `actionId`(실제 의도된 것)와 달라져 `confirm_action`이 잘못된 actionId로 실행되거나 실패.
5. 결과: HitlCard가 뜨지 않거나 예약이 완료되지 않음.

반면 **manual `bind_tools + ainvoke` 루프**는 같은 turn-2 경로에서 `prepare_*` 1회 → `start_auth` 1회로 정확하게 동작:
...
`ssu_agent/agents/library.py`, `academic.py`, `lms.py`의 `agent_node`를 `create_react_agent` 대신 수동 `bind_tools + ainvoke` 루프로 교체:
    ...
    for _llm in llm_seq:
        try:
            ...
            for _ in range(6):
                ...
                # ... tool 실행 + HITL 체크
                if hitl_triggered:
                    break  # ← prepare_*가 actionId 반환하면 즉시 중단
...
**핵심 포인트**: `hitl_triggered=True` 시 `break`으로 루프를 명시적으로 종료 → `prepare_*` 호출은 반드시 1회만 발생.

부수 효과: `config` 전파로 inner `ainvoke`가 `astream_events(version="v2")`의 `on_chat_model_stream` 이벤트를 emit → 토큰 단위 텍스트 스트리밍이 UI에 표시됨(사용자 요구 2 "중간 과정 보여주기" 충족).

`ssu_agent/supervisor/graph.py` `supervisor_node`도 단일 `create_llm()` → `get_llm_sequence()` 폴백 루프로 교체. 수퍼바이저는 turn-0 라우팅만 하므로 루핑 위험 낮아 `create_react_agent`는 유지하고 폴백 처리만 추가.

### 핵심 파일 및 커밋

- `ssuAgent/ssu_agent/agents/library.py`, `academic.py`, `lms.py` — manual bind_tools 루프 (`8e7cb07`, PR #6 → main ff)
- `ssuAgent/ssu_agent/supervisor/graph.py` — supervisor_node fallback 루프 (`8e7cb07`)
- `8e7cb07` + 포맷 커밋 `040a16b` → CI 통과 → ArgoCD 자동 배포 → pod `sha-040a16b`

### 포트폴리오 포인트

프레임워크 추상화(`create_react_agent`)를 블랙박스로 믿지 않고 **실제 동작을 A/B 테스트로 검증**했다. 예상한 XML 버그는 재현되지 않았으나, 테스트 과정에서 전혀 다른 루핑 버그를 발견했다. "가설을 사실로 기록하지 않는다"는 기록 규율도 함께 지켰다. 또한 `config` 전파라는 LangGraph 내부 동작(inner ainvoke가 astream_events에 이벤트를 emit하려면 config가 필요)을 직접 검증하고 활용한 사례.
...
### 해결 방향 (PR #72, feat/lms-export-pvc)
...
`lmsExport.pvc.enabled: true`로 설정 시 활성화 (기본값 false).
...
### 3차 수정 — 데이터 소스 피벗: 채플 페이지 → 성적표 (2026-06-20)

2차 구현도 실패했다. 사용자가 실제 채플 이수 횟수를 확인하도록 prod에서 `check_graduation_requirements`를 호출했는데 채플 항목이 여전히 `0/0 → "미이수"`를 표시.

#### 새로 발견한 진짜 원인

**`lookup()`의 의미를 잘못 이해했다.** 2차 구현은 `lookup()`을 "이전학기 버튼"으로 가정하고 `repeat(12) { ... app.lookup() ... getSelectedSemester() }` 루프를 짰다. FFI 바인딩(`rusaint_ffi.kt:1938`)을 직접 읽으니 doc comment가 "**최신 정보를 조회합니다. 새로고침 시 사용합니다**"였다. `lookup()`은 현재 학기를 **새로고침(refresh)**할 뿐, 이전 학기로 이동하지 않는다.

따라서 `getSelectedSemester()`는 매번 같은 학기를 반환 → 2차 구현의 stuck 감지 조건(`current.year == prev.year && current.semester == prev.semester`)이 **2번째 반복에서 즉시 실행** → `return@repeat`(Kotlin repeat 블록에서 `continue` 역할) → 실제론 현재 진행중 학기 1개만 읽고 종료. 현재 학기는 아직 종강 전이라 result가 "P"가 아님 → count=0 → `mergeChapelHistory`의 `if (count==0) return status` 조건에 걸려 0/0 유지.

이 과정에서 **두 가지 부가 버그**도 발견:
- `return@repeat`는 Kotlin에서 `continue`(반복 건너뜀)이지 `break`(루프 종료)가 아님. 루프는 계속 돌지만 각 반복마다 `current.year < entryYear` guard에도 걸려 사실상 아무것도 안 함.
- `mergeChapelHistory`의 `count==0` early-return이 "조회 실패"와 "진짜 0학기 이수"를 구별하지 못해 첫 학기 재학생이 `6/0`이 아닌 `0/0`으로 보이게 함.

#### 해결책

**채플 페이지(ZCMW3681, `ChapelApplication`)를 완전히 포기하고 성적표(`CourseGradesApplication`)로 피벗.**

숭실대 채플은 성적표에 학기마다 0학점 P/F 과목으로 기록된다. `CourseGradesApplication.semesters(BACHELOR)`는 이미 `get_my_grades`(`fetchGrades()`)로 검증된 경로이고, 한 번의 호출로 **전학기 누적 이력**을 반환한다. 드롭다운 선택, `lookup()`, 역방향 네비게이션이 전혀 필요 없다.

```kotlin
// 3차 구현 (RusaintUniFfiClient.kt countChapelPassedSemesters)
...
동시에 `SaintGraduationService.mergeChapelHistory`도 수정 — count 반환 시 0도 권위값으로 처리(예외만 fallback), 예외 처리를 `countCompletedChapelSemesters`에서 호출자(`mergeChapelHistory`)로 이동.

#### 핵심 파일 / 커밋

- `src/main/kotlin/com/ssuai/domain/saint/connector/RusaintUniFfiClient.kt` — `countChapelPassedSemesters` 3차 구현 (CourseGradesApplication 기반)
- `src/main/java/com/ssuai/domain/saint/service/SaintGraduationService.java` — `mergeChapelHistory` count==0 early-return 제거, 예외 처리 구조 개선
- `src/test/java/com/ssuai/domain/saint/service/SaintGraduationServiceTests.java` — 신규 케이스 `chapelGateWithZeroPassedSemestersShowsSixRequired` 추가
- `docs/adr/0049-b5-chapel-graduation-merge.md` — D3 업데이트

#### 포트폴리오 포인트

1. **라이브러리 doc을 맹신하지 않고 FFI 바인딩 소스를 직접 읽어 `lookup()` 오해를 확정** — 세 번 연속 실패 후 "코드를 더 추가"하는 대신 "기존 가정이 틀렸음"을 추적한 디버깅 방식.
2. **실패한 API 경로를 포기하고 다른 소스로 피벗** — 채플 페이지(ZCMW3681)와 성적표(CourseGrades) 두 소스의 특성(학기별 vs 누적, 드롭다운 의존 vs 세션 한 번)을 비교해 올바른 소스를 선택. "포기"가 "우회"로 이어진 사례.
3. **0을 실패 신호로 쓰는 설계 결함 수정** — 0/0이 "API 장애"인지 "진짜 0회 이수"인지 구별하지 못하던 로직을 예외/성공 반환 경로 분리로 정직하게 고침.

#### 예상 면접 질문

1. 같은 데이터를 가져오는 데 API 경로가 여러 개 있을 때(채플 페이지 vs 성적표), 어떤 기준으로 소스를 선택했나요?
2. `lookup()` 오해처럼 외부 라이브러리의 메서드 의미를 잘못 파악했다는 걸 어떻게 발견했나요? (코드 읽기 vs 로그 vs 실험)
3. "count == 0이면 실패"로 처리하던 로직이 왜 문제였고, 어떻게 예외와 정상 반환을 구별하게 바꿨나요?

---

### 4차 수정 — 배포 전 검증 스파이크로 매칭 버그 차단 (2026-06-20)

#### 새로 발견한 버그

3차 구현의 코드를 배포하기 전에 **step 0 검증 스파이크**(Claude Desktop에서 `get_my_grades` 호출)를 실행했다. 결과:

| 학기 | courseName | courseCode | score |
|---|---|---|---|
| 2022-1 | **CHAPEL** | 21501015 | P |
| 2022-2 | **CHAPEL** | 21501015 | P |
| 2025-1 | **비전채플** | 21501015 | P |
| 2025-2 | **비전채플** | 21501015 | P |

**courseName이 학기마다 다르다.** 2022년 입학 코호트는 "CHAPEL"(영문), 2025년은 "비전채플"(한글). 그런데 3차 구현의 매칭 술어는 `c.className.contains("채플")`이었다. "CHAPEL"은 "채플"을 포함하지 않으므로 **2022년 2행을 모두 놓쳐** count=4가 아닌 count=2를 반환했을 것이다. 그대로 배포했으면 게이트가 `6/2`로 틀리게 표시되는 4번째 실패가 됐을 것.

#### 해결책

**틀린 가설**: 채플 과목명에 "채플"이 항상 포함된다.  
**실제**: 과목명은 학년도마다 변동 가능. 안정 키는 **학수번호(courseCode) `21501015`** — 카탈로그 레벨 식별자라 과목명 개명에도 불변.

매칭 술어를 courseCode 우선으로 교체하고, 순수 함수로 추출해 세션 없이 CI에서 단위 테스트:
...
internal fun isChapelCourse(code: String, className: String): Boolean =
    code == CHAPEL_COURSE_CODE ||            // 안정 키 — 과목명 개명에도 불변
        className.contains("채플") ||        // 이름 보강 (한글)
        className.uppercase().contains("CHAPEL")  // 이름 보강 (영문)
```

#### 핵심 파일

- `src/main/kotlin/com/ssuai/domain/saint/connector/RusaintUniFfiClient.kt` — companion object에 `CHAPEL_COURSE_CODE` 상수 + `isChapelCourse` 술어 추가; `countChapelPassedSemesters` 내부 매칭을 `isChapelCourse(c.code, c.className)`으로 교체
- `src/test/kotlin/com/ssuai/domain/saint/connector/RusaintUniFfiClientChapelMatchTest.kt` — 신규: "CHAPEL" / "비전채플" / 코드만 / 관계없는 과목 케이스 4개
- `docs/adr/0049-b5-chapel-graduation-merge.md` — D3/D6 업데이트

#### 포트폴리오 포인트

1. **배포 전 검증 스파이크가 실제 버그를 잡은 사례** — 직전 3번의 실패는 모두 "가정을 검증 없이 배포 → prod에서 틀림" 패턴이었다. 이번엔 실제 성적 데이터로 end-to-end 검증을 먼저 하고서 bug를 배포 전에 차단했다.
2. **식별자 설계: 가변 레이블 vs 불변 코드** — 과목명은 UX 레이블이라 언제든 바뀔 수 있다. 카탈로그 레벨 학수번호(courseCode)가 안정 키이고 이름은 보완재다. 학번 대신 학수번호로 매칭하는 선택은 DB 설계의 "natural key vs surrogate key" 논의와 같은 맥락.
3. **테스트 커버리지 갭 해소** — 3번 조용히 실패한 매칭 로직이 라이브 rusaint 세션이 필요한 `RusaintUniFfiClient` 내부에 묻혀 있어 아무 테스트도 없었다. 술어를 companion 순수 함수로 추출하니 CI에서 직접 검증 가능해졌다.
...
## 사건 19: 하이브리드 RAG가 prod에서 휴면 — 가설 3개를 라이브로 배제하고 무료 quota를 특정 (2026-06-23)

라이브 실측: `search_academic_policy_sources`가 기본·`live:true` 양쪽 `embeddingUsed:false, fusionMethod:lexical`. 로그: `216 missing chunk(s); embeddingActive=false`, `academic_embeddings` 0행. 헤드라인 기능(하이브리드 RAG)의 벡터 절반이 prod에서 죽어 있었다. **플래그·키는 정상**(configmap `SSUAI_ACADEMIC_EMBEDDING_ENABLED=true` + 시크릿에 `SSUAI_GEMINI_API_KEY` 존재).

### 틀린 가설 → 라이브로 순차 배제
1. **"플래그가 꺼졌다"**(레포 application.yml 기본값 false 근거) → 기각: prod configmap이 `true`로 오버라이드(kubectl 확인).
2. **"`gemini-embedding-2-preview`가 OpenAI-호환 `/embeddings`에서 안 뜬다"** → 기각: 내 머신에서 preview·001 둘 다 단건 임베딩 **HTTP 200**.
3. **"prod egress 차단"** → 기각: prod pod에서 직접 curl(pod 키) 단건 임베딩 **HTTP 200**.

### 실제 원인 = 무료 quota + 관측성 부재
prod pod에서 **96-input 배치**(prod batch-size)를 preview로 호출 → **HTTP 429**(`quotaValue:100`, `retryDelay:52s`). 216 청크(배치 3개)가 preview의 빡빡한 quota를 넘겨 매 refresh 429 → lexical. **결정적으로 클라이언트가 예외를 클래스명만(`RestClientException`) 로깅**해서 "429 quota"라는 진짜 원인이 가려져 있었다(미진단의 직접 원인). chart가 preview를 선택한 근거(values.yaml 주석 "001 quota가 2026-06-11 crash-loop로 0 고착")는 **12일 지난 stale** — 2026-06-23 재검증 시 001은 회복(96배치 200·429 회복 ~11s).

### 해결
- 임베딩 모델 `gemini-embedding-2-preview` → **`gemini-embedding-001`**(application.yml 기본값 + chart). 001은 quota 회복 + 429 회복 ~11s.
- **배치 페이싱 교정**: free tier ~100 inputs/min이라 기본(96개/15s=192/min)은 429. **batch-size 72 + interval 65s**(3배치, 각 quota 창 분리)로 216청크가 한도 아래로 들어가게.
- **진단 로깅**: `AcademicEmbeddingClient`가 `RestClientResponseException` 시 HTTP status + 응답 본문(429 quota 메시지) 로깅(키는 응답에 없어 비노출 유지).
...
- 문서의 오래된 가정("001 quota 0 고착")을 **1차 증거(오늘의 라이브 호출)로 반증·갱신**. 단 되돌림이 옳은지 배포 후 워밍으로 검증하는 안전망 유지.
- 무료 tier quota(~100/min)에 맞춘 배치 페이싱 설계(회복시간 < 배치 간격).

### 예상 면접 질문
1. prod에서 조용히 죽은 기능을 어떻게 진단했나? (플래그/모델/egress/quota 라이브 순차 배제 + pod에서 직접 재현)
2. "A 모델은 quota 0이라 B를 쓴다"는 문서가 있는데 왜 A로 되돌렸나? (12일 stale, 1차 증거로 반증, 배포 후 검증 안전망)
3. 무료 tier quota 한계에서 216청크 코퍼스를 어떻게 안정 임베딩하나? (회복시간<배치간격 모델 선택 + 페이싱 + 429 백오프)

## 사건 20: 공급망 SHA-pin 커밋이 Dockerfile FROM 인라인 주석으로 자기 배포를 막아 prod가 2026-06-22부터 stale (2026-06-23)

2026-06-23 W1(보안) 머지 후 main 이미지 빌드가 **`FROM requires either one or three arguments`(line 14)**로 실패. 확인하니 **공급망 SHA-pin 커밋(`d99a155`, ADR0062) 이래 6/22부터 모든 main 이미지 빌드가 실패**하고 있었다 → ArgoCD가 새 이미지를 못 받아 **prod가 마지막 성공 빌드(공급망 하드닝 직전)에 고착** = stale. 공급망 하드닝(`d99a155`) 자체도 배포된 적이 없었다(MASTERPLAN의 "전부 배포" 기록은 머지=배포로 오인).

### 틀린 가설 → 실제 원인
- (초기) "방금 머지한 W1이 빌드를 깼다" → **기각**: W1은 Dockerfile을 안 건드림. `git log -- Dockerfile`로 `d99a155`가 범인.
- **실제**: `d99a155`가 digest 핀과 함께 FROM 3줄에 **인라인 주석**(`FROM img@sha256:... AS name  # img`)을 붙였다. Dockerfile은 **라인 중간 주석을 지원하지 않아**(주석은 줄 시작에서만) buildkit이 `# img` 토큰을 FROM 인자로 파싱 → 인자 개수 오류. 정적 파싱 에러라 매 빌드 실패.

### 해결
인라인 주석 제거(태그는 digest ref `img:tag@sha256:...`에 이미 포함) + 사람용 태그는 FROM **앞줄 주석**으로 이동. 3개 스테이지(rusaint-builder/builder/runtime) 전부. 커밋 `a20d84a`.

### 검증
머지 후 main 이미지 빌드 **success** → ArgoCD Image Updater가 새 태그 커밋 → 새 backend pod 배포(0-restart)·health UP·`/api/admin/resilience` 무인증 401. 6/22 이래 미배포였던 공급망 하드닝 + W1 + 후속이 함께 배포됨.

### 포트폴리오 포인트
- **"tests green but prod broken"의 정석**: 백엔드 테스트·gitleaks는 통과하는데 **이미지 빌드(배포 파이프라인)가 조용히 깨져** prod가 stale. CI의 빌드 잡 실패를 배포 게이트로 감시하지 않으면 "머지=배포"라는 착각에 빠진다.
- **공급망 하드닝 커밋이 자기 배포를 막은 아이러니** — 보안 강화(SHA 핀)가 문법 오류로 배포 자체를 무력화. 변경 후 실제 빌드·배포·prod 헬스까지 확인하는 것의 가치.
...
## 사건 22: batch 축소가 드러낸 잠복 디코딩 버그 — Gemini가 첫 item의 index를 생략, Jackson 3 primitive int가 200 응답을 통째로 폐기 (2026-06-29)

사건 21 수정(batch 8 + 증분 영속) 배포 후 새 pod에서 워밍이 **여전히 0행**. 단, 에러가 바뀌었다: 사건 21까지의 429(TPM)가 아니라 **`RestClientException` (no HTTP response body)**, 첫 배치(offset 0)부터. 즉 quota가 아닌 다른 실패.

### 틀린 가설 → 라이브로 순차 배제
1. **egress/DNS 차단** → 기각: 새 pod 안에서 curl로 임베딩 엔드포인트 **HTTP 200**(DNS·connect 정상).
2. **키/배치크기/콜드스타트 일시성** → 기각: pod 안 curl(키+8배치)=200, **JDK HttpClient(Spring RestClient가 쓰는 바로 그 클라이언트)로 직접 호출=HTTP/2·1.1 둘 다 200**(190KB 수신). 강제 refresh로 12:50에 재현 = 일시성 아님.
3. **의존성/코드 변경** → 기각: `git diff eafeaac..b25a770` 결과 `callEmbeddings`·생성자·base-url **전부 불변**(내 변경은 embed() 반환의미+주석+batch 기본값뿐).

### 실제 원인 (전체 스택트레이스로 확정)
로컬 gradle에서 prod와 동일한 spring-web/Jackson으로 `client.callEmbeddings(한글 8청크)` 직접 호출 → 정확한 예외 획득:
```
RestClientException: Error while extracting response for type [EmbeddingResponse] ...
Caused by: HttpMessageNotReadableException: JSON parse error: Cannot map `null` into type `int`
Caused by: tools.jackson.databind.exc.MismatchedInputException
```
- **Gemini의 OpenAI-호환 `/embeddings`는 응답 배열 첫 item(index 0)의 `index` 필드를 생략**한다(라이브 확인: item0 keys=`[object,embedding]` index MISSING, item1=`[object,index,embedding]`). 0을 falsy로 보고 누락시키는 듯.
- **`EmbeddingResponse.Item` 레코드의 `int index`(primitive)**에 누락→null 매핑 시도 → **Jackson 3**(`tools.jackson` = Spring Boot 4, `FAIL_ON_NULL_FOR_PRIMITIVES` 기본 거부)이 `MismatchedInputException` → 200 응답 **통째로 폐기** → `embeddingActive=false` 영구.
- **왜 지금까지 잠복?** 옛 pod(batch=72)는 매 refresh **항상 429**(TPM, 사건21)라 디코딩 경로에 도달조차 못 함. 사건21의 batch=8이 처음으로 200을 받아내자 **한 번도 실행된 적 없던 성공-경로의 버그가 발현**. (사실 `index`는 코드에서 미사용 — embed()는 `item.embedding()`만 읽고 순서로 벡터-입력 대응.)

### 해결
`record Item(List<Double> embedding, int index)` → **`record Item(List<Double> embedding)`** (unused `index` 제거). `@JsonIgnoreProperties(ignoreUnknown=true)`가 응답의 `index`/`object`를 무시. 회귀 테스트: 첫 item이 index를 생략한 prod-형태 JSON을 `JsonMapper`로 역직렬화 → 성공 단언. **라이브 확정**: 고친 `embed()` 전체 경로를 실제 API로 호출 → 8×768 벡터 정상 반환 + `AcademicVectorCodec.encode`까지 통과(성공-경로 전 구간 검증).
...
## 사건 26: Spring Boot 4.1.0의 Jackson 3 전환이 jjwt(Jackson 2) claim 역직렬화를 깨뜨려 로그인 refresh가 401이 된 문제 (2026-06-30)

u-SAINT SSO 로그인은 성공하고 브라우저에 refresh cookie도 저장·전송됐지만, `POST /api/auth/refresh`가 계속 401을 반환했다. 직전 의존성 bump 커밋 `b923464`가 Spring Boot 4.0.6을 4.1.0으로 올렸고, 이 버전 조합에서 `JwtProvider.parse`의 `Jwts.parser().parseSignedClaims()`가 claim 역직렬화 중 실패해 `InvalidJwtException`으로 감싸졌다. 즉 쿠키·서명·issuer·학생 존재 여부가 아니라 JWT 파서 런타임 호환성 문제가 prod login down의 최종 원인이었다.

### 틀린 가설

1. 처음에는 "Boot 4.1.0이 `ResponseEntity` 반환 경로에서 `Set-Cookie`를 떨어뜨린다"는 가설을 세웠다. `SaintSsoCallbackController`에서 쿠키를 `ResponseEntity` 헤더에 직접 붙이는 핫픽스(`abe13db`)를 적용했지만 prod 401은 해결되지 않았다.
2. 그 다음에는 서명키·시크릿 불일치 또는 cookie 전송 문제를 의심했다.
3. denylist, issuer/type claim, DB 학생 존재 여부 문제도 후보로 봤다.

### 실제 원인

브라우저 Network에서 SSO callback 응답이 cookie를 set하고, refresh 요청이 그 cookie를 다시 보내는 것을 확인했다. 토큰을 수동 HMAC 검증했을 때 서명도 통과했고, issuer/type claim도 기대값과 일치했으며, 학생 row도 DB에 존재했다. Redis denylist는 fail-open 성격이라 401의 직접 원인이 아니었다.

남은 차이는 런타임 파서였다. `b923464`의 Spring Boot 4.1.0 bump는 관리 Jackson 계열을 Jackson 2(`com.fasterxml.jackson`)에서 Jackson 3(`tools.jackson`) 중심으로 이동시켰다. 반면 현재 JWT 라이브러리는 `jjwt 0.13.0`의 `jjwt-api`/`jjwt-impl`/`jjwt-jackson` 조합이고, `jjwt-jackson`은 Jackson 2에 의존한다. 이 불일치로 `parseSignedClaims()` claim 역직렬화가 실패했고, `JwtProvider.parse`가 이를 `InvalidJwtException`으로 래핑하면서 `AuthController.refresh`가 401을 반환했다.

별도 잠복 버그도 발견했다. prod `SSUAI_JWT_SECRET` 값 끝에 Windows artifact인 trailing `\r`가 있었다. raw-bytes fallback 덕분에 현재 서명 검증은 일관되게 통과했지만, 운영 secret은 `\r`를 제거하고 `JwtProvider.buildSigningKey`도 base64 decode 전 `.strip()`으로 방어하는 후속이 필요하다.

### 해결

의존성 bump 커밋 `b923464` 전체를 revert해 Spring Boot 4.0.6, Kotlin 2.3.21, Redisson 4.5.0, Gradle wrapper 9.5.1, Resilience4j 2.3.0, OkHttp 5.3.2, JNA 5.18.1 및 이전 GitHub Actions pin으로 복구한다. `SaintSsoCallbackController`의 SSO cookie 핫픽스(`abe13db`)는 별도 문제 완화로 유지한다.
...
- 직접 의존하지 않은 Jackson major 전환이 managed dependency를 통해 `jjwt-jackson` 런타임을 깨뜨린 공급망 회귀를 live evidence로 좁혔다.
- 쿠키 저장/전송, 프록시, HMAC 서명, secret bytes, issuer/type claim, DB row, denylist, 라이브러리 역직렬화 경로를 순서대로 소거해 인증 장애를 진단했다.
- "처음 그럴듯한 프레임워크 쿠키 가설"이 틀렸을 때 문서화하고, 같은 증상을 더 깊은 런타임 호환성 문제로 재분류했다.
...
## 사건 28: 로그인 장애의 진짜 원인은 jjwt/Jackson이 아니라 refresh-token reuse-denylist였던 문제 (2026-06-30)

ssuAI 로그인이 안 되는 증상이 **여러 세션에 걸쳐 jjwt/Jackson 회귀로 오진**됐다. u-SAINT SSO 인증은 성공하는데 직후 `POST /api/auth/refresh`가 401을 반환해 사용자가 로그인 화면으로 다시 튕겼다.

### 틀린 가설 (3겹)

1. **(핸드오프)** Spring Boot 4.1.0의 Jackson 3 전환이 `jjwt-jackson`(Jackson 2)을 깨뜨려 `parseSignedClaims`가 실패 → revert로 Boot 4.0.6 복원. **그래도 안 됨.**
2. **(이번 세션)** 그럼 Boot 4.0.6에서도 jjwt가 Jackson과 충돌하나 → `jjwt-jackson`을 `jjwt-gson`으로 교체해 JWT를 Jackson과 완전 디커플(PR #147). **그래도 안 됨.**
3. 헤드리스 검증(엔드포인트가 401 JSON을 깨끗이 반환·로그 클린)을 "복구됨"으로 단정 → **실제 브라우저 E2E를 안 한 게 화근.**

### 실제 원인

사용자 devtools에서 401 응답과 **쿠키의 실제 refresh 토큰**을 확보해 소거법으로 좁혔다.

1. **토큰을 prod 시크릿으로 오프라인 HMAC-SHA256 검증** → 서명이 `base64decode(secret.strip())`(32바이트) 키와 **정확히 일치**. 즉 서명·키·jjwt는 정상(가설 1·2가 틀렸음을 수학적으로 증명). 부수 발견: prod `SSUAI_JWT_SECRET` 끝에 Windows artifact `\r`(0x0d)가 있었고, #147의 `.strip()` 하드닝이 키를 base64-decode 경로로 바꿔 서명이 그 키로 생성·검증되고 있었다.
2. claim도 정상(iss=ssuai, typ=REFRESH, jti 존재, 미만료), `students` 테이블에 학생 존재.
3. 남은 건 `AuthController.refresh`의 **denylist 검사**뿐 → Redis에서 `ssuai:auth:refresh-denylist:<jti>` 키가 **존재**함을 확인(수십 개 누적).

원인은 **refresh-token rotation의 reuse-denylist**였다. SSO 콜백이 refresh T1을 발급→쿠키 set. 프론트가 `/api/auth/refresh`로 T1을 1회 소비하면 새 T2를 발급하고 **T1의 jti를 denylist에 등록(reuse 차단)**한다. 그러나 (a) rotated 쿠키 T2가 cross-site(Vercel rewrite proxy)에서 브라우저의 기존 쿠키를 안정적으로 대체하지 못하고 (b) `/auth/return` 페이지가 refresh를 1회 이상 호출할 수 있어, 브라우저가 **이미 denied된 T1을 재전송** → denylist hit → 401 → 로그인 튕김.

### 해결

`AuthController.refresh`에서 reuse-denylist의 **검사(`isDenied`)와 등록(`deny`)을 제거**했다. refresh token은 TTL(14일) 내 재사용을 허용한다(사용자가 보안 무관심을 명시 — refresh reuse 수용 가능). rotation 쿠키 set은 유지(붙으면 sliding, 안 붙어도 기존 토큰이 동작). logout의 쿠키 클리어는 그대로(쿠키 삭제로 로그아웃은 유효).

### 핵심 파일 / 커밋

- `src/main/java/com/ssuai/domain/auth/controller/AuthController.java` — refresh의 denylist check/deny 제거
- `src/test/java/com/ssuai/domain/auth/controller/AuthControllerTests.java` — `refreshReturns401WhenRefreshJtiDenied` → `refreshAcceptsAReusedRefreshToken`로 전환
- 오진 우회(유지): revert `95834be`(Boot 4.0.6), PR #147 `1d9f98e`(jjwt-gson + secret strip) — 둘 다 해롭진 않으나 진짜 원인은 아니었음

### 포트폴리오 포인트

- **그럴듯한 첫 가설(공급망/프레임워크 회귀)에 매몰되지 않고, 사용자에게서 실제 토큰을 받아 오프라인 서명검증 → claim검증 → Redis/DB 상태검증으로 소거법으로 진짜 원인을 좁힌 사례.** 서명이 키와 일치한다는 한 줄 증명이 두 개의 큰 가설을 동시에 반증했다.
- **헤드리스 검증(엔드포인트 401)의 한계**: 유효 토큰 round-trip(=실제 E2E)을 안 하면 "정상 401"과 "버그 401"을 구분 못 한다. prod 인증 장애는 반드시 실제 자격증명으로 E2E 해야 한다.
- refresh-token rotation + reuse-detection은 보안엔 좋지만 **cross-site 쿠키 rotation이 불안정하면 정상 사용자를 잠근다** — 보안과 가용성의 트레이드오프.

### 예상 면접 질문

1. 로그인 401을 jjwt/Jackson으로 오진했다가 어떻게 반증했나? (사용자 토큰을 prod 시크릿으로 오프라인 HMAC 검증 → 서명 일치 = 키/파서 정상 증명)
2. refresh-token rotation의 reuse-denylist가 왜 정상 사용자를 401시켰나? (rotated 쿠키가 cross-site에서 기존 쿠키를 대체 못 함 + 중복 호출 → denied된 토큰 재전송)
3. rotation을 유지하면서 이 문제를 풀려면? (denied 직후 짧은 grace window로 reuse 허용, 또는 쿠키 rotation을 신뢰성 있게 — 본 건은 보안 무관심이라 denylist 제거가 가장 단순)
