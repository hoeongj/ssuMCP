...
계획대로 revert 커밋 `95834be`(Spring Boot 4.0.6 복구)가 main에 머지되고 배포까지 완료됐다. prod pod는 `ghcr.io/hoeongj/ssumcp:sha-95834be`로 교체됐고, Ready 상태에 restart 0회, health 엔드포인트는 UP을 반환했다. 인증 엔드포인트는 더 이상 500이 아니라 정상 401을 반환했고(파서 런타임 호환성 회귀 해소), pod 로그에도 `InvalidJwtException` 스택트레이스가 더 이상 찍히지 않았다. 즉 `b923464` 회귀가 완전히 해소되어 로그인 refresh 경로가 정상 복구됐다.
