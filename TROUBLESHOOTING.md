...
- 해결:
  - `image-build` job 조건을 바꿔 수동 실행(`workflow_dispatch`)과 일반 `main` push는 허용하되, 커밋 메시지가 `build: automatic update of ssuai-backend`로 시작하는 자동 write-back commit은 이미지 빌드를 건너뛰게 했다.
  - 첫 조건식은 `!startsWith(...)` 형태로 작성했지만 GitHub Actions가 workflow 평가 단계에서 job을 만들기 전에 실패했다. 조건을 `${{ ... startsWith(...) == false }}` 형태로 바꿔 동일한 의미를 유지하면서 파서 해석을 명시화했다.
...
- 핵심 파일/커밋:
  ...
  - 해결 커밋: 이 기록이 포함된 `fix(ci): skip image build for image updater commits`
- 검증:
  - 로컬에서 관련 SSE registry 테스트와 전체 테스트를 실행한다.
  - 해결 커밋의 첫 image-build는 정상 실행되어 새 이미지를 만든다.
  - 그 뒤 Image Updater가 마지막으로 한 번 더 write-back commit을 만들 수 있지만, 해당 자동 커밋에서는 `image-build` job이 skipped가 되어 추가 이미지 태그가 생기지 않아야 한다.
  - ArgoCD Application이 `Synced/Healthy`, Deployment rollout 성공, `/actuator/health` `UP`이면 수렴으로 판단한다.
- 포트폴리오 포인트:
  - GitOps 자동화는 "배포 상태를 Git에 기록"하는 장점이 있지만, CI가 모든 Git 커밋을 동일하게 취급하면 자동화끼리 피드백 루프를 만들 수 있다.
  - 해결 포인트는 Image Updater를 끄는 것이 아니라, 빌드 산출물을 만들어야 하는 커밋과 배포 상태만 갱신하는 커밋을 CI 조건에서 분리하는 것이다.
- 면접 예상 질문:
