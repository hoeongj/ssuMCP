...
| 프로바이더별 LLM 키 — `SSUAI_GEMINI_API_KEY`, `SSUAI_GROQ_API_KEY`, ... (10개 프로바이더, privacy 모드는 gemini 제외 9개) | Secret | 라이브 (채팅). 각 선택적; 키 없으면 해당 프로바이더 건너뜀 |
...
서버는 조회 쿼리·인증 세션 도구와 함께 **학교 상태를 바꾸는 도서관 좌석 action 도구**(`prepare_reserve_library_seat`·`prepare_swap_library_seat`·`prepare_cancel_library_seat` + 공용 `confirm_action`, 그리고 장기 대기형 `wait_for_library_seat`)를 노출한다. 이 섹션은 그 action들이 반드시 통과해야 하는 게이트를 정의한다. 이 정책을 이끄는 **플래그십 action 도구**는 도서관 좌석 예약이며, 미래형 단일 `reserve_library_seat`이 아니라 `prepare_reserve_library_seat` + `confirm_action` 2단계로 구현되어 있다 ([ssuAI vision](https://github.com/hoeongj/ssuAI/blob/main/docs/vision.md) 참조).
...
학교 상태를 변경하는 도구(도서관 `prepare_*` + `confirm_action`, LMS export write, 향후 u-SAINT write)는 반드시:
...
- 도서관 좌석 예약·이석·반납 action 도구가 노출되어 있으며, 모두 `prepare_*` + `confirm_action` 2단계 확인과 `action_audit` 감사를 거친다(§6, [ADR 0015](adr/0015-action-tool-infrastructure.md)). 예약 confirm은 동의 증적을 `action_audit`에 남긴 뒤 intent 큐로 실행한다. LMS 자료 내보내기도 `prepare_lms_material_export`/`export_all_lms_materials` + `confirm_lms_material_export` 2단계다. u-SAINT write는 아직 없다.
