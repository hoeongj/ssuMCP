...
현재 서버는 조회 쿼리와 인증 세션 수명 도구만 노출한다. 아직 학교 상태를 변경하는 action을 노출하지 않으므로, 이 섹션은 향후 추가될 action을 위한 필수 게이트를 정의한다. 이 전체 정책을 이끄는 **플래그십 action 도구**는 `reserve_library_seat`이다 ([ssuAI vision](https://github.com/ghdtjdwn/ssuAI/blob/main/docs/vision.md) 참조). 아래 규칙이 전부 충족되기 전까지 그 도구는 출시할 수 없다.
