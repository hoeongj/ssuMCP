...
| 프로바이더별 LLM 키 — `SSUAI_GEMINI_API_KEY`, `SSUAI_GROQ_API_KEY`, ... (10개 프로바이더) | Secret | 라이브 (채팅). 각 선택적; 키 없으면 해당 프로바이더 건너뜀 |
...
- 도서관 웹 로그인 키는 서블릿 세션 id가 아니라 서버가 로그인 시 발급한 `ssuai_library_session` HttpOnly 쿠키다. 쿠키 `maxAge`는 `LibrarySessionStore` TTL과 맞춘다(ADR 0096).
...
학교 상태를 변경하는 도구(`prepare_*` + `confirm_action`, LMS 자료 내보내기, 향후 u-SAINT write)는 반드시:
...
action 도구는 공유 `action_audit` 테이블로 감사한다(출시 완료). 전체 스키마, `PREPARED → EXECUTING → SUCCESS/FAILURE_*` 상태 머신, `prepare_X` + `confirm_action` MCP 형태는 [ADR 0015](adr/0015-action-tool-infrastructure.md)에 고정되어 있다. 이 문서가 보안 기준 문서로 남을 수 있도록 간략하게 요약하면:

- action당 하나의 행이 `PENDING → EXECUTING →` 종료 상태까지 원위치에서 변경된다. `dry_run_preview`는 사용자에게 보여준 정확한 텍스트를 기록한다.
- **stale action 방지(ADR 0055)**: 같은 owner가 새 액션을 prepare하면 직전 PENDING 액션들은 단일 UPDATE로 `SUPERSEDED`(종결·실행 불가)로 전이된다. 따라서 owner당 활성 PENDING은 최대 1건이며, 두 번째 `confirm_action`이 사용자가 다시 승인하지 않은 오래된 액션을 실행할 수 없다.
- **소유권·명시 ID 강제(ADR 0055)**: `confirm_action`은 선택적 `action_id`를 받는다. 지정 시 `id AND student_id AND status=PENDING` WHERE 절 행 락으로 **해당 액션만** 확정하고, 타 owner·미존재·이미 처리/만료는 거부하며 다른 액션으로 폴백하지 않는다. 생략 시 활성 PENDING이 0건이면 안내, 다수면 거부(action_id 요구), 1건이면 그 액션을 ID로 확정한다.
...
- **JWT를 불투명 세션 대신 선택.** 이 문서의 초안에서는 취소 가능성 때문에 불투명 Redis 기반 세션을 선호했지만, Task 14의 u-SAINT SSO 흐름이 Redis 도입 전에 출시해야 했고, ssuAI는 재사용 가능한 학교 비밀번호를 저장하지 않는다 (즉시 폐기하는 일회용 SmartID 토큰만 수신). 따라서 탈취된 access JWT의 피해 범위는 15분 TTL로 제한되어 있어, 대부분의 격차가 해소된다. refresh JWT는 로그아웃 시 `jti` denylist에 올리지만, refresh 재사용 차단은 cross-site rotation 장애를 만든 실제 원인이라 제거했다.
...
- **클라이언트에서 토큰이 존재하는 곳** — refresh JWT는 `HttpOnly`, `Secure`, `Path=/api/auth` 쿠키. SSO 콜백은 이 쿠키를 직접 싣지 않고, same-origin `POST /api/auth/exchange` 또는 `/api/auth/refresh`의 200 응답만 쿠키를 내려준다(ADR 0095). Access JWT는 프론트엔드 메모리에만 존재 (localStorage·sessionStorage 절대 안 됨). 모바일은 동일한 엔드포인트에 동일한 `Authorization: Bearer` 형태를 사용한다.
...
- 공개 CORS mapping은 `allowCredentials(false)`다. 직접 origin 호출은 학식·기숙사 식단, 공지, 시설, 학사일정, 도서관 좌석 상태/SSE, 도서 검색처럼 로그인 없는 GET/SSE만 허용한다. refresh cookie, library `ssuai_library_session`, SmartID/LMS Bearer, 예약/대출, MCP web session, agent stream은 ssuAI same-origin proxy path를 유지한다.
...
- 학교 상태를 변경하는 action 도구(도서관 예약·이석·반납, LMS 자료 내보내기)는 출시되었으며, §6의 `prepare_*` + `confirm_action` 2단계 확인과 `action_audit` 감사 요구사항을 모두 강제한다.

---

## 14-1. 보안 하드닝 (2026-06 remediation)

> 여러 독립적인 보안 리뷰에서 모인 지적을, **코드 대조로 진짜/오진/이미고침을 가른 뒤** wave 단위로 반영한 결과다. 오진 판정·signoff 근거는 각 항목의 ADR에, 미해결 항목은 `docs/security-followups.md`에 정리했다. 아래는 **배포 완료된 제어(live control)**의 색인이며, 각 항목의 배경·대안·근거·동작 원리는 링크된 ADR에 있다(여기서 중복하지 않는다).

이 절은 색인이다. 같은 제어를 더 깊게 다루는 기존 절(§5 자격증명, §6 action 확인, §7 인증·세션, §8 전송)과 충돌하지 않으며, 거기서 빠진 "이번 remediation에서 새로 추가/강화된 제어"만 한 줄씩 요약·링크한다.

| 제어 | 무엇 + 왜 | 근거 |
|------|-----------|------|
| **SSO callback authorization-code 교환** | u-SAINT SSO 콜백은 더 이상 `Set-Cookie`를 싣지 않는다. Redis 1회용 code(TTL 120s, 단일 사용, fail-closed)를 URL로 전달하고, same-origin `POST /api/auth/exchange`의 비리다이렉트 200 응답만 refresh 쿠키를 발급한다. 프론트/인그레스/CDN의 다중 `Set-Cookie` 파싱·redirect 처리 차이에 로그인 성공 여부를 걸지 않는다. | [ADR 0095](adr/0095-sso-authorization-code-exchange.md) |
...
| **OAuth-sub 소유권 가드 + state 원자 소비** | 세션 해소(`McpAuthHelper.resolveSession`) Tier2/3에서 기존 바인딩된 oauth-sub와 불일치하면 세션을 반환하지 않는다(bind-or-verify: null→바인딩, 일치→통과, 불일치→deny). 타인 transport/opaque id를 알아낸 공격자가 자기 JWT로 피해자 세션에 sub를 덮어쓰는 세션 고정/권한 상승을 차단. OAuth state는 JPQL `deleteIfActive` 행수 claim으로 1회만 소비(find-then-delete race 제거). 부수 수정: 개인 도구 OK 응답이 입력 인자 대신 **해소된 canonical 세션 id + provider**를 반환(외부 리뷰 오진을 유발한 echo·`provider:null` 착시 제거). | [ADR 0056](adr/0056-mcp-oauth-ownership-guard-state-consume.md) |
...
- **(보류) 멀티 인스턴스 세션 스토어·워커 동시성** — 멀티 레플리카 전환 시 outbox/export worker claim(`FOR UPDATE SKIP LOCKED`)·rate-limit 분산 카운터(Redis)·세션 스토어 내구성/취소를 함께 재검토한다. 현재 `replica=1`이라 보류 → `docs/security-followups.md` #4.
---

## 16. MCP 세션 인증 — 2-mode 아키텍처 (ADR 0036)

> 상세 설계 결정 근거: `docs/adr/0036-mcp-auth-optin-two-mode.md`

### 왜 2가지 모드가 필요한가

외부 MCP 클라이언트(예: ChatGPT)는 LLM 응답의 인자(argument)로 `mcp_session_id`를 유지해야 한다. ChatGPT는 턴 경계에서 이 값을 드랍하는 경우가 있어, 매 호출마다 새 orphan 세션이 생성되고 로그인이 항상 필요한 **무한 AUTH_REQUIRED 루프**가 발생할 수 있다. 이를 해결하기 위해 두 가지 신원 운반 레이어를 선택적으로 사용한다.

### 기본 모드 (mcp_session_id 기반 + transport 바인딩)

모든 클라이언트의 기본 동작. 변경사항 없음:

1. `start_auth(provider)` 호출 → `mcp_session_id` 발급 + 브라우저 로그인 URL 반환.
2. 사용자가 브라우저에서 로그인 완료.
3. 이후 모든 private 도구 호출에 `mcp_session_id` 포함.

**Transport session binding(ChatGPT 루프 수정):** `start_auth` 실행 시, 서버는 현재 HTTP 연결의 `Mcp-Session-Id` 헤더(transport-layer 식별자)를 세션에 바인딩한다. 이후 LLM이 `mcp_session_id`를 드랍해도 transport ID가 안정적으로 유지되면 서버가 같은 세션을 찾을 수 있다. 이 바인딩은 **서버가 연결에 직접 귀속**시키므로 크로스-유저 하이재킹이 없다. (opaque ID 병합과의 차이: 클라이언트 제공 값을 신뢰하지 않음.)

### 옵트인 OAuth RS 모드 (rs-enabled=false 기본)

> `SSUAI_OAUTH_RS_ENABLED=true` + `SSUAI_OAUTH_ISSUER_URI` + `SSUAI_OAUTH_AUDIENCE` 설정 시 활성화.
> 기본값은 모두 비활성 — 기존 동작 완전 보존.

- MCP 클라이언트가 HTTP 계층에서 Bearer JWT를 전달하면, 서버는 이를 검증하고 `SecurityContextHolder`에서 `sub`(구글 사용자 고유 ID)를 읽는다.
- `sub`는 LLM 인자가 아닌 **HTTP 헤더에서 온다** → LLM이 드랍할 수 없음 → 세션이 대화를 넘어 안정적으로 유지됨.
- **학교 비밀번호 저장 없음:** OAuth 모드는 신원 고정만 한다. 학교 시스템(u-SAINT/LMS/도서관) 로그인은 여전히 브라우저 플로우 1회 필요. 세션 TTL 내에서는 재로그인 없이 동작("자동로그인" 효과). 학교 비밀번호는 이 모드에서도 저장하지 않는다.
- **기술 구현:** `spring-boot-starter-oauth2-resource-server` + 커스텀 `McpOAuthSecurityConfig`. `/mcp/**` 는 `permitAll()` — 토큰 없으면 anonymous 통과(원래 모드), 토큰 있으면 검증·SecurityContext 채움, 만료·무효면 401. audience 검증 강제 (`JwtClaimValidator<List<String>>`).
- **체인 스코프 (ADR 0074):** OAuth 리소스 서버 필터는 **두 SecurityFilterChain으로 분리**돼 있고, OAuth 체인은 `securityMatcher("/mcp","/mcp/**","/.well-known/**")`로 **MCP 표면에만 한정**된다(@Order(1)). 나머지 경로(REST·`/api/auth/**`·chat)는 permissive 2번 체인(@Order(2))이 받고 웹 세션 인증은 서블릿 `JwtAuthFilter`가 담당한다. 이유: `BearerTokenAuthenticationFilter`는 *인증* 필터라 `permitAll()`(인가 규칙)과 무관하게 Bearer가 보이면 무조건 Auth0 디코더로 검증한다 — 스코프 없는 단일 체인이면 prod(`rs-enabled=true`)에서 ssuAI 자체 HS256 웹 세션 JWT를 "Auth0 토큰 아님"으로 401시켜 u-SAINT 로그인 후 개인화 API 전체가 죽는다(2026-07-03 실제 prod 장애). 회귀 가드 = `McpOAuthChainScopingTests`(WireMock OIDC로 `rs-enabled=true` 부팅해 웹 JWT `/me` 200 · `/mcp` challenge 유지 · PRM `authorization_servers` 유지 동시 검증).
- **RFC 9728 PRM:** `/.well-known/oauth-protected-resource` 노출 → OAuth 클라이언트의 AS 자동 발견. ⚠️ Spring Security 7은 이 문서를 **자동 등록된 `OAuth2ProtectedResourceMetadataFilter`로 직접 서빙**한다(같은 경로의 수제 `@GetMapping`은 필터가 먼저 실행돼 가려짐). 기본 문서엔 외부 AS를 알 수 없어 `authorization_servers`가 빠지므로, `protectedResourceMetadata(...).protectedResourceMetadataCustomizer(...)`로 issuer를 주입해야 ChatGPT가 Auth0를 발견한다 (ADR 0037, 없으면 `start_auth` 무한 루프).
- **ChatGPT OAuth (Auth0 DCR) 운영 설정 (ADR 0038):** ChatGPT 커넥터는 DCR로 등록되는 **third-party 앱**이라 Auth0 쪽에 3가지가 필요하다 — ① API 식별자 = `https://ssumcp.duckdns.org/mcp`(= ChatGPT가 RFC 8707 `resource`로 보내는 커넥터 URL)이고 서버 `SSUAI_OAUTH_AUDIENCE`도 동일해야 토큰 `aud`가 검증을 통과 ② **default client grant**(`default_for:third_party_clients`)로 모든 DCR 클라이언트에 API 접근 허용(매 연결마다 새 client_id라 per-client grant는 무용) ③ 로그인 connection을 **domain connection**(`is_domain_connection:true`)으로 승격. 서버엔 Auth0 관리 자격증명 불필요(공개 JWKS로 JWT만 검증) — 위 설정은 Management API로 1회 수행. 상세: TROUBLESHOOTING 2026-06-18 "Auth0 DCR 5단 관문".

### 3-tier 세션 해석 순서 (`McpAuthHelper.resolveSession`)

모든 private 도구 호출에서 인증 세션을 다음 순서로 탐색한다:

1. **Tier 1 — OAuth sub** (`SecurityContextHolder`): `JwtAuthenticationToken.getName()` → `oauthSubject` 컬럼 조회. OAuth 모드에서만 유효.
2. **Tier 2 — Transport session ID** (`Mcp-Session-Id` 헤더): 서버가 연결에 귀속시킨 transport ID → `transportSessionId` 컬럼 조회. ChatGPT 루프 수정에 핵심.
3. **Tier 3 — Opaque mcp_session_id** (LLM 인자): 기존 방식. Tier 1·2 실패 시 fallback.

각 tier에서 OAuth sub가 존재하면 기회적 바인딩(opportunistic bind) — 향후 Tier 1 조회를 가능하게 하고자 현재 세션에 sub를 기록한다.

### 보안 불변 조건

| 조건 | 이유 |
|------|------|
| 학교 비밀번호(u-SAINT/LMS/도서관)는 저장하지 않는다 | OAuth 모드도 신원 고정만; 학교 자격증명 저장 없음 |
| `Mcp-Session-Id`는 서버가 발급·귀속 — 클라이언트가 임의 값 주입 시 기존 세션 없음 | transport 바인딩이 opaque ID 병합보다 안전한 이유 |
| `sub`는 HTTP 헤더, `Mcp-Session-Id`도 HTTP 헤더 — 두 tier 모두 LLM이 조작 불가 | Tier 1·2가 Tier 3보다 우선하는 이유 |
| RS 모드 비활성 시 JWT 검증 완전 OFF → oauth 클래스 자체 미생성 | `rs-enabled=false` 기본값의 의미 |
