...
- **stale action·owner isolation(ADR 0086·0098)**: MCP library prepare는 같은 `(MCP session owner, action type, target key)`의 직전 PENDING만 `SUPERSEDED`(종결·실행 불가)로 전이한다. 다른 대상의 action은 공존할 수 있으므로 `confirm_action`은 여러 건일 때 추측하지 않고 `ACTION_CONFLICT`와 명시 `action_id`를 요구한다. claim은 정확한 MCP session owner·PENDING·TTL을 row lock 아래 재검증하므로 foreign/stale action이 실행되지 않는다. legacy web/LMS 경로의 owner-wide supersede는 action-id targeting 부재를 위한 별도 호환 규칙이다.
...
- MCP private 도구는 해당 프로바이더가 연동될 때까지 `AUTH_REQUIRED`를 반환한다. 층별 집계만 반환하는 `get_library_seat_status`는 내부 sampler token을 쓰는 공개 read이며, 예약·추천·내 좌석·대출 같은 사용자별 도구는 계속 LIBRARY 연결을 요구한다.
- 채팅 턴에서 private 도구를 호출하면, 결과 합성과 이후 대화 히스토리는 공개 프로바이더 풀 대신 설정된 private LLM 프로바이더 정책을 사용한다.
- 도서관 층별 좌석 캐시는 `floor + upstream token 존재 여부`로 구분한다. 좌석 수는 전역 aggregate라 production의 내부 sampler와 사용자 token 호출은 authenticated category를 공유하되, token 원문·사용자 식별자·개인 좌석 정보는 캐시 값과 key에 포함하지 않는다.
...
### 명시 ID 우선 세션 해석 (`McpSessionResolver`, ADR 0098)

일반 private tool은 nonblank `mcp_session_id`를 정확한 권한 경계로 취급한다. 해당 세션이 없거나
만료·무효면 `INVALID_SESSION`이고, 현재 OAuth/transport binding과 다르면 `SESSION_MISMATCH`다.
두 경우 모두 다른 세션으로 fallback하거나 session ID·login URL을 응답에 넣지 않는다.

명시 ID가 없을 때만 현재 `Mcp-Session-Id` binding을 사용하고 없으면 `NO_SESSION`이다. OAuth subject는
이 binding의 소유권을 검증할 뿐, 일반 작업에서 새 binding을 만들지 않는다. `start_auth`만 명시 rebind
컨텍스트에서 binding을 변경할 수 있다.
...
| 명시 ID는 exact match이며 transport보다 우선 | 잘못된 explicit ID가 현재 연결의 다른 개인 세션으로 fallback하지 않음 |
| `Mcp-Session-Id`와 OAuth sub는 생략 호출의 binding 소유권만 증명 | ordinary tool이 암묵적으로 rebind하지 않음 |
