...
- **SSE intent IDOR 차단 + 세션 회전** — 예약 intent SSE(wait/events)에서 `intent.sessionKey == caller` 검증, 불일치 404(타인 intent 진행상황 누수 차단) + SSE registry per-intent 상한·빈키 정리. 도서관 로그인 성공 시 `changeSessionId`로 HTTP 세션 회전(인증/바인드 분리, 기존 `login()` 보존 → MCP/sampler 무영향).
- **Refresh denylist Redis 영속(P1-Q)** — §7 참조. 무효화 `jti`를 Redis에 잔여 수명 TTL로 저장(JVM 재시작·다중 인스턴스 재사용 차단), Redis blip 시 fail-open.
