...
> 5종 AI(Claude·ChatGPT·Gemini·AGY·Codex)에게 전체 코드를 주고 받은 보안 분석을, **코드 대조로 진짜/오진/이미고침을 가른 뒤** wave 단위로 반영한 결과다. 통합 triage·오진 판정·signoff 전문은 `../SECURITY_REMEDIATION_PLAN.md`, 미해결 항목은 `docs/security-followups.md`. 아래는 **배포 완료된 제어(live control)**의 색인이며, 각 항목의 배경·대안·근거·동작 원리는 링크된 ADR에 있다(여기서 중복하지 않는다).
...
| 제어 | 무엇 + 왜 | 근거 |
|------|-----------|------|
| **OAuth-sub 소유권 가드 + state 원자 소비** | 세션 해소(`McpAuthHelper.resolveSession`) Tier2/3에서 기존 바인딩된 oauth-sub와 불일치하면 세션을 반환하지 않는다(bind-or-verify: null→바인딩, 일치→통과, 불일치→deny). 타인 transport/opaque id를 알아낸 공격자가 자기 JWT로 피해자 세션에 sub를 덮어쓰는 세션 고정/권한 상승을 차단. OAuth state는 JPQL `deleteIfActive` 행수 claim으로 1회만 소비(find-then-delete race 제거). 부수 수정: 개인 도구 OK 응답이 입력 인자 대신 **해소된 canonical 세션 id + provider**를 반환(ChatGPT 오진을 유발한 echo·`provider:null` 착시 제거). | [ADR 0056](adr/0056-mcp-oauth-ownership-guard-state-consume.md) |
...
| **공급망 재현성 + k8s pod-security** | GitHub Actions를 40자 commit SHA로 핀(태그 재지정 공격 무력화), Gradle wrapper jar sha256 검증, Docker base image `@sha256:` digest 핀. k8s pod에 `seccompProfile:RuntimeDefault` + `automountServiceAccountToken:false` + 권한상승 차단/capabilities drop. **NetworkPolicy·read-only rootfs는 미적용**(오설정 시 prod 아웃 위험·자율 검증 불가 → `docs/security-followups.md`). | [ADR 0062](adr/0062-supply-chain-k8s-pod-security.md) |

같은 remediation에서 함께 들어간, 별도 ADR이 없는 제어(상세는 위 §·`SECURITY_REMEDIATION_PLAN.md`):
