...
공개 집계를 제공했지만, 같은 데이터의 MCP `get_library_seat_status`는 사용자 LIBRARY 세션을
요구했다. 이 불일치 때문에 로그인 없이 답할 수 있는 “5층 빈자리” 질문이 채팅에서 불필요한
로그인 URL로 전환됐다.

MCP 도구와 `/api/chat`의 해당 tool dispatch도 공개 service 경계로 정렬하고
`mcp_session_id` 인자를 제거한다. 응답은 전체 또는 compact 좌석 집계 DTO만 반환하며 sampler
token은 도구 schema·응답·로그에 나타나지 않는다. 공개 좌석 결과는 채팅의 private-provider
sticky 판정에서도 제외한다.
