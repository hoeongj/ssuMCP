...
*   **만료 시간 설정**: `SseEmitter`에 55초(`55,000ms`) 타임아웃을 지정하여 Vercel rewrite 프록시의 120초 하드 컷보다 먼저 정리합니다. 초기 구현에서는 30분으로 잡았지만, dead emitter가 30분 동안 레지스트리에 쌓이는 문제가 확인되어 짧은 타임아웃으로 조정했습니다.
...
*   **버퍼링 방지 헤더**: `/api/library/seats/events` 응답에 `X-Accel-Buffering: no`를 추가하여 nginx 계열 프록시가 SSE 스트림을 버퍼링하지 않도록 합니다. 버퍼링이 켜져 있으면 좌석 변경 이벤트가 즉시 전달되지 않고 묶음 전송처럼 보일 수 있습니다.
*   **하트비트 주기**: `@Scheduled(fixedDelay = 20_000)` 하트비트를 보내 중간 프록시가 유휴 연결을 끊는 상황을 방지합니다. 20초는 일반적인 idle timeout보다 충분히 짧으면서도 불필요한 트래픽을 과도하게 늘리지 않는 절충값입니다.
*   **예외 격리**: 특정 클라이언트 송신 실패(`IOException` 등)가 발생해도 다른 Emitter 전송이나 애플리케이션 전체에 영향을 주지 않도록 루프 내에서 예외를 캐치해 대상 Emitter만 정리합니다.

## 버그 기록

- 최초 구현에서는 `SseEmitter` 타임아웃을 30분으로 두었다. 이 설정은 브라우저 재연결 빈도를 줄이는 쪽으로 보였지만, 실제 운영 경로에서는 Vercel rewrite 프록시가 외부 연결을 120초에 하드 컷하면서 오래된 emitter가 레지스트리에 남아 누적되는 역효과가 있었다.
