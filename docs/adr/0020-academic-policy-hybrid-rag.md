...
## 2026-07-02 Amendment — 후속 과제 2건 구현 완료

Consequences의 "can add contentHash-based skip-unchanged later"와 2026-06-18 Amendment의 후속 과제 **배치 단위 점진 영속**이 모두 `PersistentAcademicEmbeddingStore`로 구현 완료됐다(2026-06-29부터 prod 라이브). `(chunk_hash, model)` 키로 기존 영속 벡터를 스킵하고, 임베딩 클라이언트가 반환한 prefix 분량은 429로 중단되더라도 즉시 영속되므로 콜드 부트스트랩이 여러 갱신 회차에 걸쳐 점진적으로 corpus를 데운다 — 한 회차 all-or-nothing 한계 해소.

---

## 2026-07-09 Amendment — pgvector 확장이 prod에 실제로 활성화됨 (2026-06-18 개정의 "확장 부재" 전제 갱신)

- **상태**: SSH 실측 확인. 코드 변경 없음(본 개정은 사실관계 갱신).

### 갱신 배경

2026-06-18 Amendment는 pgvector 기각 사유 #1로 "prod Postgres(외부 stock 17.10)에 `vector` 확장 **부재 실측**(`pg_available_extensions` 빈 결과)"을 들었다. 이후 prod DB가 `pgvector/pgvector:pg17` 이미지로 교체되며 이 전제가 바뀌었다(배경은 ADR 0070 2026-07-09 개정 참고). SSH로 직접 확인한 현재 prod 상태:

- `vector` 확장 설치·활성화됨(버전 `0.8.4`).
- `academic_embeddings.embedding_vec` 컬럼에 **217행 전량 벡터 값이 채워짐**.
- HNSW 코사인 인덱스(`idx_academic_embeddings_vec`)가 존재(#151에서 추가).

### 무엇이 superseded 되고 무엇이 안 바뀌었나

- **superseded**: "prod에 `vector` 확장이 없다"는 2026-06-18 Amendment의 기각 사유 #1은 더 이상 사실이 아니다. `deploy/charts/ssuai-backend/values.yaml`도 `springProfilesActive: prod,pgvector,json-logs`로 pgvector 프로파일을 prod에 명시적으로 켜 두고 있다(ADR 0070이 만든 옵션 경로가 이제 prod에도 적용된 상태).
- **안 바뀐 것**: 라이브 RRF 검색은 여전히 본 ADR의 기본 경로(`PersistentAcademicEmbeddingStore` 인메모리 코사인)를 그대로 쓴다. `PgVectorAnnIndex`(ADR 0070)를 호출하는 서비스 코드는 없다(IT 테스트 전용 호출자만 존재) — 즉 prod DB에는 ANN 인덱스와 벡터 데이터가 이미 준비돼 있지만, **쿼리 서빙 경로는 아직 인메모리 코사인**이다. 코퍼스가 217청크 규모에 머무는 한 서빙 경로를 바꿀 성능상 실익은 없다.

리뷰어가 "pgvector 미사용/불가"로 오독하지 않도록: 정확한 현재 상태는 "확장·인덱스·데이터는 prod에 이미 존재, 서빙 경로는 여전히 인메모리"다.
