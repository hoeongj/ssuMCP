...
|---|---|
| 날짜 | 2026-06-30 |
| 상태 | **Superseded (2026-07-10)** — 여기서 정의한 graduate 트리거가 캠퍼스-규모 시나리오로 충족되어 Kafka로 승격됨. 본 ADR은 "그 시점의 판단 근거"로 유효하게 보존. → **ADR 0089**(KRaft 브로커), **ADR 0090**(툴콜 이벤트 파이프라인), **ADR 0091**(예약 intent-status 버스 graduation) |
| 범위 | 좌석/예약 이벤트 전파(Redisson `RTopic`, PG `LISTEN/NOTIFY`), 예약 intent SSE fan-out |
| 연관 문서 | ADR 0047(좌석 분산 락), ADR 0048(intent SSE·Redis fan-out), ADR 0055(confirm 상태머신), ADR 0089/0090/0091(graduation), 내부 스펙 문서 §SET D |

> 스펙 초안은 0069로 적었으나 번호 충돌로 0071로 재배정(0069=관측성, 0070=pgvector).
>
> **2026-07-10 후속**: 이 ADR은 "분당 수백 건·<5 소비자" 규모에서 Kafka를 **미도입**하고 graduate 트리거를 명문화한 결정이다. 이후 시나리오가 "전교생 수만 명 공식 캠퍼스 MCP"로 상향되어 그 트리거가 충족 → 계획대로 Kafka로 승격했다(0089/0090/0091). 아래 본문은 **당시 근거**로 그대로 보존한다(면접에서 "왜 처음엔 안 썼고 언제·어떻게 도입했나"의 전반부 근거).
