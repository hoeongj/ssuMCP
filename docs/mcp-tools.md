...
외부 MCP client (Claude Desktop, Cursor 등) 도 `mcp_session_id` 기반 인증 세션을 통해 `get_my_schedule`, `get_my_grades`, `get_my_assignments`, `get_library_seat_status`, `get_my_library_loans` 를 직접 호출할 수 있다. 인증이 없으면 `AUTH_REQUIRED` 응답과 로그인 URL 을 반환한다.
...
[ssuAI vision](https://github.com/ghdtjdwn/ssuAI/blob/main/docs/vision.md) 참고.
