...
| `get_meal_weekly` | 캠퍼스 식당 주간(7일) 메뉴. `weekOffset`으로 주 단위 이동 | `weekOffset` (선택; 0=이번 주·1=다음 주·-1=지난 주, `[-8, 8]`로 클램프) | `WeeklyMealResponse` |
...
```json
{
  ...
  "loginUrl": "https://api.ssuai.example/api/mcp/auth/saint/start?state=...",
  "expiresAt": "2026-05-18T12:10:00Z",
  "message": "AUTHENTICATION REQUIRED. Action needed: 1) Open this exact raw loginUrl in a browser and complete login: <loginUrl> 2) Show the same raw loginUrl as visible text to the user … 3) After login, retry this exact tool call with mcp_session_id.",
  "userMessage": "로그인이 필요해요. 아래 링크를 브라우저에서 열어 로그인한 뒤 같은 요청을 다시 해주세요: <loginUrl>",
  "developerMessage": "AUTHENTICATION REQUIRED. Action needed: 1) … (message와 바이트 동일)",
  "data": null
}
```
...
개인(LMS):
  ...
  prepare_lms_material_export, export_all_lms_materials,
...
## 8. 위험·write tool 정책
현재 노출된 52개 MCP tool 중 대부분은 read-only다. 세션 상태를 변경하는 도구는
...
- `confirm_action` 은 pending action lookup → row-lock claim → 실행 → audit 상태 전이를 담당한다. 예약 action은 PR2부터 직접 Pyxis를 호출하지 않고 immediate `library_reservation_intents` row를 만든 뒤 worker 결과를 최대 약 8초 폴링한다. 반납/이석 action은 아직 직접 upstream 호출 경로다.
