...
도서 검색 응답의 `hasNext`/`totalPages`는 새 record 컴포넌트가 아니라 `@JsonProperty` 파생 메서드로 노출한다.
이렇게 하면 기존 생성자·호출부·테스트 fixture를 바꾸지 않고, Jackson 직렬화 결과에만 페이지 보조 필드를 포함할 수 있다.
...
| tool name | 설명 | 주요 인자 |
| --- | --- | --- |
| `get_auth_status` | 현재 MCP 세션의 provider 연결 상태 조회. 명시 `mcp_session_id`가 있으면 정확히 그 세션만, 없을 때만 transport binding으로 탐색. `status`: `OK` / `INVALID_SESSION` / `SESSION_MISMATCH` / `NO_SESSION`. 각 provider는 `linked`, `health`, 검증·성공·실패 시각, `failureCode`, `credentialVersion`을 반환 | `mcp_session_id` (선택) |
...
#### 세션 해석 — 명시 ID 우선 (ADR 0098)

`mcp_session_id`는 선택 입력이다. 값이 있으면 서버는 **그 정확한 세션만** 사용한다. 존재하지 않거나
무효면 `INVALID_SESSION`, 현재 transport binding과 다르면 `SESSION_MISMATCH`이며 다른 세션으로
fallback하지 않는다. 값이 없을 때만 현재 MCP transport에 안전하게 바인딩된 세션을 사용하고, binding이
없으면 `NO_SESSION`이다. `start_auth`만 명시 rebind를 허용한다.

거부 응답은 session ID·login URL을 반환하지 않는다. 유효하지만 해당 provider가 미연결인 정확한 세션에만
`AUTH_REQUIRED`와 로그인 URL이 반환된다. `mcp_session_id`는 secret으로 취급해 로그·문서·대화에 남기지
않는다.

### 2c. Private tool (인증 필요)

`mcp_session_id` 는 선택 입력이다. 해당 provider가 연결된 정확한 resolved session이면 `status: "OK"`와 데이터를 반환한다. 정확한 세션만 유효하지만 provider가 미연결일 때만 `AUTH_REQUIRED`와 login URL을 반환한다. 명시 ID가 무효·만료면 `INVALID_SESSION`, transport와 다르면 `SESSION_MISMATCH`, 생략했고 binding이 없으면 `NO_SESSION`이다.
...
| 상태/코드 | 의미 | 세션·로그인 URL 노출 |
| --- | --- | --- |
| `OK` | 요청이 성공했고 `data`에 결과가 있음 | 해소된 세션만 반환 가능 |
| `AUTH_REQUIRED` | 정확한 세션은 있으나 provider 미연결 또는 upstream 인증 필요 | 해당 세션과 provider login URL만 반환 |
| `INVALID_SESSION` / `SESSION_MISMATCH` / `NO_SESSION` | 해소 거부 | 둘 다 반환하지 않음 |
...
- `prepare_X` 가 정확한 dry-run 문구를 반환하고 `action_audit` 에 MCP session owner가 포함된 `PENDING` row 를 기록한다. 같은 `(owner, actionType, targetKey)`의 직전 PENDING만 `SUPERSEDED`로 전이된다. 서로 다른 대상의 action은 공존할 수 있으므로 owner-wide supersede를 가정하면 안 된다.
- `confirm_action` 은 (지정 시) `action_id` 의 MCP session owner·PENDING·미만료를 행 락 WHERE 절로 재검증해 그 액션만 실행하고, 타 owner·미존재·이미 처리/만료는 `NO_PENDING_ACTION`으로 거부하며 다른 액션으로 폴백하지 않는다. 생략 시 활성 PENDING 0건→`NO_PENDING_ACTION`, 다수→`ACTION_CONFLICT`와 action_id 요구, 1건→그 액션 확정이다. 예약 action은 immediate `library_reservation_intents` row를 만들고 즉시 접수 응답을 반환한다. 최종 결과는 `get_library_wait_status` 또는 SSE로 확인하며, MCP 호출 스레드는 worker를 폴링하지 않는다. 반납/이석은 직접 upstream 호출 경로다.
