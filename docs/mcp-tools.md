...
외부 MCP client (Claude Desktop, Cursor 등) 도 `mcp_session_id` 기반 인증 세션을 통해 `get_my_schedule`, `get_my_grades`, `simulate_gpa`, `get_my_assignments`, `get_library_seat_status`, `recommend_library_seats`, `prepare_reserve_library_seat`, `get_my_library_seat`, `prepare_swap_library_seat`, `prepare_cancel_library_seat`, `get_my_library_loans` 를 직접 호출할 수 있다. 인증이 없으면 `AUTH_REQUIRED` 응답과 로그인 URL 을 반환한다.
...
| `get_library_seat_catalog` | 도서관 정적 좌석·열람실 카탈로그 | `floor_code`, `room_code`, `include_layout` (선택) | `LibrarySeatRoomCatalogResponse` |
...
| tool name | 설명 | 주요 인자 | 응답 DTO |
| --- | --- | --- | --- |
| `get_academic_calendar` | 학사 일정 조회 | `year`, `month` (선택) | `AcademicCalendarResponse` |
...
| `get_library_seat_status` | 도서관 층별 좌석 현황 | LIBRARY | `floor` (2/5/6), `mcp_session_id` |
| `recommend_library_seats` | 선호도 기반 좌석 추천. 현재 live availability와 정적 좌석 카탈로그를 결합 | LIBRARY | `floor`, `window`, `outlet`, `standing`, `edge`, `quiet`, `near_entrance`, `limit`, `mcp_session_id` |
| `prepare_reserve_library_seat` | 좌석 예약 준비. 실제 예약은 `confirm_action` 필요 | LIBRARY | `mcp_session_id`, `seat_id` |
| `get_my_library_seat` | 현재 예약 좌석 조회 | LIBRARY | `mcp_session_id` |
| `prepare_swap_library_seat` | 기존 예약 좌석에서 새 좌석으로 이석 준비. 실제 이석은 `confirm_action` 필요 | LIBRARY | `mcp_session_id`, `seat_id` |
| `prepare_cancel_library_seat` | 현재 예약 좌석 반납 준비. 실제 반납은 `confirm_action` 필요 | LIBRARY | `mcp_session_id`, `charge_id` |
...
개인(LIBRARY):
  get_library_seat_status, recommend_library_seats, get_my_library_loans,
  prepare_reserve_library_seat, prepare_cancel_library_seat,
  get_my_library_seat, prepare_swap_library_seat, confirm_action
```
...
## 8. 위험·write tool 정책
현재 노출된 33개 MCP tool 중 대부분은 read-only다. 세션 상태를 변경하는 도구는
`start_auth`, `logout_provider`, `logout_all`이고, 학교 시스템 상태를 바꾸는 도서관
좌석 action은 반드시 `prepare_*` + `confirm_action` 2단계로만 실행된다.
...
제품 완성 기준과 남은 작업은 [library-seat-agent-completion-plan.md](library-seat-agent-completion-plan.md)를 따른다.
