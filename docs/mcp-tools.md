...
| `search_academic_policy_sources` | 공식 학칙·졸업·장학 출처 근거 검색. `live=true`면 공식 URL 즉시 조회. 응답에 `liveRequested`(호출자 입력)·`liveExecuted`·`corpusType`(live/mixed/seed) 메타데이터 포함 | `query`, `category`, `limit`, `live` | `AcademicPolicySearchResponse` |
...
| `get_my_assignments` | 현재 학기 미제출 과제·퀴즈 목록. 비어 있으면 `message`로 안내 | LMS | `mcp_session_id` |
