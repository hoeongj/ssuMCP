...
- Tags produced by `docker/metadata-action@v6`:
  - `ghcr.io/hoeongj/ssuai-backend:latest`
  - `ghcr.io/hoeongj/ssuai-backend:sha-<full-sha>`
...
- The `Backend image (ghcr.io, ARM64)` job pushed:
  - `ghcr.io/hoeongj/ssuai-backend:latest`
  - `ghcr.io/hoeongj/ssuai-backend:sha-8f0cd37b87aefc2f9b23bccb6e5a453e03d40376`
...
```text
gh api '/users/hoeongj/packages/container/ssuai-backend/versions?per_page=5'
...
- Confirm the secret named
  ...
  token with `contents:write` on `hoeongj/ssuAI`.
