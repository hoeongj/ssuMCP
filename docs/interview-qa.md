...
> 이 5개 문항의 공통 메타-서사: **트렌디한 도구를 막 넣은 게 아니라, 규모에 맞는 판단(안 쓸 근거와 graduate 트리거를 먼저 문서화하고, 캠퍼스 규모 시나리오가 트리거를 만족하자 Kafka·pgvector로 계획대로 승격)·구조적으로 못 잡는 것을 잡는 테스트(Testcontainers·EmbeddedKafka·전체 컨텍스트 flag-ON 회귀)·도메인 의미를 담은 계측(LLM 스팬)·가설에 매몰되지 않는 소거법 디버깅**을 보여준다. 모든 결정은 `docs/adr/`(88개), 대표 장애 디버깅은 [`docs/troubleshooting-highlights.md`](troubleshooting-highlights.md)(12대 사례)에 "틀린 가설 → 실제 원인 → 해결 + 면접질문"까지 기록돼 있다.
