...
> 이 5개 문항의 공통 메타-서사: **트렌디한 도구를 넣은 게 아니라, 규모에 맞는 판단(Kafka/pgvector 거부)·구조적으로 못 잡는 것을 잡는 테스트(Testcontainers)·도메인 의미를 담은 계측(LLM 스팬)·가설에 매몰되지 않는 소거법 디버깅**을 보여준다. 모든 결정은 `docs/adr/`(69개), 모든 장애는 `TROUBLESHOOTING.md`(28건)에 "왜·대안·면접질문"까지 기록돼 있다.
