...
**핵심**: 규모 판단이다. 학칙 코퍼스는 217청크라 인메모리 exact 코사인(~1ms)이 충분하고 HNSW 이점이 발현 안 되며, prod Postgres엔 `vector` 확장이 아예 없다(`CREATE EXTENSION`이 부팅을 깬다). 그래서 **옵션 `pgvector` 프로파일 + Testcontainers(pgvector/pg16) IT로 ANN 동작을 격리 증명**하되 prod 기본 마이그레이션 경로엔 신규 migration 0개로 둬서 prod 스키마·동작은 불변. "트렌디한 도구를 넣는 것"보다 **"언제 안 쓰는지를 판단하고, 필요해질 때를 위해 증명해 둔 것"**이 더 강한 신호라고 봤다.
