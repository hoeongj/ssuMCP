...
### Q9. "이벤트 기반인데 왜 Kafka를 안 썼나?" (ADR 0071)
...
> 이 5개 문항의 공통 메타-서사: **트렌디한 도구를 넣은 게 아니라, 규모에 맞는 판단(Kafka/pgvector 거부)·구조적으로 못 잡는 것을 잡는 테스트(Testcontainers)·도메인 의미를 담은 계측(LLM 스팬)·가설에 매몰되지 않는 소거법 디버깅**을 보여준다. 모든 결정은 `docs/adr/`(75개), 대표 장애 디버깅은 [`docs/troubleshooting-highlights.md`](troubleshooting-highlights.md)(11대 사례)에 "틀린 가설 → 실제 원인 → 해결 + 면접질문"까지 기록돼 있다.
